...
class Observer {
  ...
  recv (message) {
    ...
    if (result === EVENT_TYPES.ACK) {
      ...
      this.responses.get(id)(error, result);
    }
  ...
  sequence (attempts = 0) {
    if (this.preflight === TIMERS.CONN_FULFILLED) {
    ...
    } else if (attempts < TIMERS.MAX_CONN_ATTEMPTS) {
      ...
      return Promise.reject(
        new Error(`Observable storage instance at ${this.seanceOrigin} cannot be reached`)
      );
    }
  }
